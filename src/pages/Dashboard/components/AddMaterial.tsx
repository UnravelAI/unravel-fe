...
const AddMaterial = ({ courses, isOpen, setIsOpen, refreshMaterials }: { courses: any[], isOpen: boolean, setIsOpen: (value: boolean) => void, refreshMaterials: () => void }) => {
    ...
    return (
        <Modal isOpen={isOpen} style={modalStyle} shouldCloseOnOverlayClick={true} onRequestClose={() => setIsOpen(false)}>
            <div style={{ backgroundColor: "#327d6b", padding: 30 }}>
                ...
                <h3 style={{ color: "white" }}>New Material</h3>
            </div>
            <div style={{ display: "flex", flexDirection: "column", padding: 30 }}>
                <Controller
                    ...
                    render={({ onChange, value }) => (
                        <div style={{ display: 'flex', flex: 1, flexDirection: "column" }}>
                            <InputLabel id="course" style={{ marginBottom: 5 }}>Course</InputLabel>
                            <Select
                ...
                <Controller
                    ...
                    defaultValue=""
                    render={({ onChange, value }) => (
                        <TextField style={{ marginTop: 10 }} error={errors?.title} helperText={errors?.title?.message} id="outlined-basic" label="Title *" variant="outlined" size="small" onChange={onChange} value={value} />
                    )}
                />
                <Controller
                    name="description"
                    ...
                    defaultValue=""
                    render={({ onChange, value }) => (
                        <TextField error={errors?.description} helperText={errors?.description?.message} id="outlined-basic" label="Description" variant="outlined" size="small" onChange={onChange} value={value} style={{ marginTop: "10px" }} />
                    )}
                />
